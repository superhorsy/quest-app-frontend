...
export const QuestExecution = () => {
    ...
    const {quests, totalQuestsCount, isLoading, error} = useSelector(state => state.questExecutionReducer)
            ...
            <Stepper activeStep={activeStep} orientation="vertical">
                {quests.map(({id, label, description, hasAnswer, rightAnswer}) => (
                    <Step key={id}>
                        <StepLabel
                            icon={reqId}
                            StepIconComponent={(props) => ColorLibStepIcon(props, {hasAnswer, rightAnswer, id})}
                        >
                            {label}
                        </StepLabel>
                        <StepContent>
